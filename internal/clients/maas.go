...
import (
	...
	clusterv1beta1 "github.com/Marouan-chak/provider-upjet-maas/apis/cluster/v1beta1"
	namespacedv1beta1 "github.com/Marouan-chak/provider-upjet-maas/apis/namespaced/v1beta1"
)
...
const (
	...
	errUnmarshalCredentials = "cannot unmarshal MAAS credentials as JSON"
	errMissingAPIKey        = "apiKey is required in credentials"
	errMissingAPIURL        = "apiURL is required in ProviderConfig"
)
...
func TerraformSetupBuilder(version, providerSource, providerVersion string) terraform.SetupFn {
	return func(ctx context.Context, client client.Client, mg resource.Managed) (terraform.Setup, error) {
		...
		if pcSpec.APIURL == "" {
			return ps, errors.New(errMissingAPIURL)
		}
		...
		// Validate apiKey is present in credentials
		apiKey, ok := creds["apiKey"]
		if !ok || apiKey == "" {
			return ps, errors.New(errMissingAPIKey)
		}

		// Set MAAS provider configuration
		ps.Configuration = map[string]any{
			"api_url": pcSpec.APIURL,
			"api_key": apiKey,
		}

		// Set optional fields
		if pcSpec.APIVersion != "" {
			ps.Configuration["api_version"] = pcSpec.APIVersion
		}
		if pcSpec.InstallationMethod != "" {
			ps.Configuration["installation_method"] = pcSpec.InstallationMethod
		}
